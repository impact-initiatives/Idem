...
```{r gt-helper, include = FALSE}
# Shared gt styling used throughout — keeps chunk code DRY
fmt_results <- function(tbl, title, subtitle) {
  tbl |>
    gt() |>
    ...
    tab_style(
      style     = cell_fill(color = "#fce8e8"),
      locations = cells_body(rows = severity == "error")
    ) |>
    tab_style(
      style     = cell_fill(color = "#eaf4ea"),
      locations = cells_body(rows = nrow(tbl) == 0L)
    ) |>
    tab_style(
      style     = cell_text(weight = "bold"),
      locations = cells_column_labels()
    ) |>
    cols_width(detail ~ px(340)) |>
...
```{r validate-all-gt, echo = FALSE}
fmt_results(
  issues,
  title    = "**Validation results**",
  subtitle = md("All checks — `target` vs `dev`")
)
```
...
```{r check-question-names-gt, echo = FALSE}
fmt_results(
  result_qn,
  title    = "**Check: `question_names`**",
  subtitle = "Questions present in target but absent from dev"
)
```
...
```{r check-list-names-gt, echo = FALSE}
fmt_results(
  result_ln,
  title    = "**Check: `list_names`**",
  subtitle = "Choice lists defined in target's choices but absent from dev's choices"
)
```
...
```{r check-survey-list-names-gt, echo = FALSE}
fmt_results(
  result_sln,
  title    = "**Check: `survey_list_names`**",
  subtitle = "Lists referenced in target's survey but absent from dev's survey"
)
```
...
```{r check-choices-gt, echo = FALSE}
fmt_results(
  result_ch,
  title    = "**Check: `choices`**",
  subtitle = "Choice options present in target but absent from dev, for shared lists"
)
```
