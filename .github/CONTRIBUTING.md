...
are made in the *source* file. This generally means editing
[roxygen2 comments](https://roxygen2.r-lib.org/articles/roxygen2.html) in an
`.R` file, not a `.Rd` file. You can find the `.R` file that generates a given
`.Rd` by reading the comment in the first line of the `.Rd` file.

## Bigger changes

If you want to make a bigger change, it's a good idea to file an issue first
...
a unit test, if needed). The tidyverse guide on
...
- New code should follow the [tidyverse style guide](https://style.tidyverse.org).
  The `air-format` hook applies this automatically on commit — please don't
  restyle code that is unrelated to your change.
